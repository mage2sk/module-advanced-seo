...
namespace Panth\AdvancedSEO\Model\Admin;
...
class AiButtonRenderer
{
    ...
    {
        return $this->seoConfig->isEnabled() && $this->seoConfig->isAiEnabled();
    }

    /**
     * Build a UI component meta array for the AI generate container.
     *
     * @param string $entityType    Entity type identifier (e.g. 'faq', 'testimonial')
     * @param string $idFieldName   The form input name that holds the entity ID
     * @param string $storeFieldName The form input name for store ID (or empty)
     * @param array  $fieldMap      Map of AI field key => form input name
     * @param array  $perFieldConfig Per-field config for individual AI buttons
     * @param string $uniqueSuffix  Unique suffix for DOM IDs to avoid conflicts
     * @param string $placeholderHelp Placeholder help text for the prompt textarea
     * @param int    $sortOrder     Sort order in the fieldset
     * @return array UI component meta array
     */
    ...
    /**
     * Load saved AI prompts from the database.
     *
    ...
    /**
     * Build the full HTML + JS block for the AI generate button.
     */
...
<div style="margin:8px 0 12px;padding:12px 15px;background:#f0f7ff;border:1px solid #c8ddf4;border-radius:4px;">
    ...
    <div style="margin-top:10px;">
        <button type="button" id="panth-seo-ai-generate-btn-{$sfx}"
            onclick="panthSeoAiGenerate_{$sfx}(this)"
        ...
        <span id="panth-seo-ai-status-{$sfx}" style="margin-left:12px;font-size:12px;color:#666;"></span>
    </div>
...
(function(){
    ...
    /* --- Generate All Fields --- */
    window['panthSeoAiGenerate_' + S] = function(btn) {
        var entityId = getEntityId();
        var storeId = getStoreId();
        var statusEl = document.getElementById('panth-seo-ai-status-' + S);
