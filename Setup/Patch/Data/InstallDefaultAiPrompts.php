 ...
 *
 * This patch is fully idempotent: each prompt is keyed by its unique `name` column, and a
 * SELECT is performed before INSERT so re-running setup:upgrade (or re-running this patch
 * after clearing its row from patch_list) will never create duplicates. Rows that were
...
class InstallDefaultAiPrompts implements DataPatchInterface
{
    ...
    public function apply(): self
    {
        ...
        if (!$connection->isTableExists($table)) {
            // Table created via db_schema.xml; skip if not yet applied
