...
namespace Panth\AdvancedSEO\Controller\Adminhtml\AiGenerate;
...
/**
 * AJAX controller for on-page AI meta generation.
 */
class Generate extends Action implements HttpPostActionInterface, CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        // Sanitize images: accept array of base64 strings, max 5
        if (!is_array($images)) {
            $images = [];
        }
        $images = array_slice(array_filter($images, 'is_string'), 0, 5);

        // Validate entity type (core + third-party Panth module types)
        $allowedTypes = [
        ...
        try {
            ...
            // Add prompt selection to context
            if ($customPrompt !== '') {
                // User edited or wrote a custom prompt — use it directly
                $context['custom_prompt'] = $customPrompt;
            } elseif ($promptId > 0) {
                $context['prompt_id'] = $promptId;
            }

            // Inject fields into context for multi-field prompt building
            $context['fields'] = $fields;

            // Pass uploaded images to AI generator (optional, for vision analysis)
            if (!empty($images)) {
            ...
            // Extract only the requested field values
            $data = [];
            ...
            // Fallback: map legacy keys to new field names
            ...
            $this->logger->error('Panth SEO AI Generate controller error: ' . $e->getMessage(), [
                'entity_type' => $entityType,
                'entity_id'   => $entityId,
                'exception'   => $e,
            ]);

            // Sanitize error message: only expose safe, known error types to the client
            $safeMessage = 'An unexpected error occurred. Please check the system logs.';
            $msg = $e->getMessage();
            // Allow through common user-actionable messages (no sensitive internals)
